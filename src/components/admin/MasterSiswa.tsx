...
import { api, getCurrentStoredUser } from '../../services/api';
...
export const MasterSiswa: React.FC = () => {
  ...
  const [bulkDeleteConfirmedCheckbox, setBulkDeleteConfirmedCheckbox] = useState(false);
  const [bulkDeleteConfirmationText, setBulkDeleteConfirmationText] = useState('');
  const [bulkDeleteStudentCount, setBulkDeleteStudentCount] = useState(0);
  ...
  const openBulkDeleteModal = async (initialClassId?: string) => {
    const targetId = initialClassId || selectedClassId || classes[0]?.id || '';
    setBulkDeleteClassId(targetId);
    setBulkDeleteConfirmedCheckbox(false);
    setBulkDeleteConfirmationText('');
    setBulkDeleteError('');

    if (targetId) {
      try {
        const res = await api.getStudents({ class_id: targetId });
        setBulkDeleteStudentCount(res.students?.length || 0);
      } catch (err) {
        const cls = classes.find((c) => c.id === targetId);
        setBulkDeleteStudentCount(cls?.total_students || 0);
      }
    } else {
      setBulkDeleteStudentCount(0);
    }
    setBulkDeleteModalOpen(true);
  };

  const handleBulkDeleteClassChange = async (targetId: string) => {
    setBulkDeleteClassId(targetId);
    setBulkDeleteConfirmationText('');
    setBulkDeleteConfirmedCheckbox(false);
    setBulkDeleteError('');
  ...
  const handleExecuteBulkDelete = async () => {
    if (!isAdmin) {
      showError('Akses ditolak: Hanya Administrator yang berwenang.');
      return;
    }

    const targetClass = classes.find((c) => c.id === bulkDeleteClassId);
    if (!targetClass) {
      setBulkDeleteError('Silakan pilih kelas terlebih dahulu.');
      return;
    }

    const expectedName = targetClass.class_name.trim();
    if (bulkDeleteConfirmationText.trim() !== expectedName) {
      setBulkDeleteError(`Ketik "${expectedName}" sama persis untuk mengonfirmasi.`);
      return;
    }

    if (!bulkDeleteConfirmedCheckbox) {
      setBulkDeleteError('Centang persetujuan terlebih dahulu.');
      return;
    }
    ...
    try {
      const res = await api.bulkDeleteStudentsByClass(bulkDeleteClassId);
      showSuccess(res.message);
      setBulkDeleteModalOpen(false);
      loadData();
    } catch (err: any) {
      const msg = err.message || 'Gagal menghapus data siswa.';
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isAdmin && (
            <button
              id="btn-bulk-delete-class-modal"
              onClick={() => openBulkDeleteModal()}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold text-xs shadow-xs transition"
              title="Hapus Semua Siswa per Kelas"
            >
              <Trash2 className="w-4 h-4 text-rose-600" />
              <span>Hapus Siswa per Kelas</span>
            </button>
          )}
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          {isAdmin && selectedClassId && (
            <button
              onClick={() => openBulkDeleteModal(selectedClassId)}
              className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-xs transition shrink-0"
              title="Hapus seluruh siswa di kelas yang difilter ini"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>Hapus Semua Siswa Kelas Ini</span>
            </button>
          )}
        </div>
      </div>
      ...
      {/* Bulk Delete Students per Class Modal */}
      {bulkDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-xs animate-in fade-in">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 sm:p-7 shadow-2xl border border-rose-100 animate-in zoom-in-95">
            {/* Modal Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 leading-tight">
                    Hapus Semua Siswa per Kelas
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Operasi pembersihan siswa satu rombel kelas sekaligus
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Pilih Kelas yang Ingin Dikosongkan:
                </label>
                <select
                  value={bulkDeleteClassId}
                  onChange={(e) => handleBulkDeleteClassChange(e.target.value)}
                  disabled={isBulkDeleting}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500"
                >
                  <option value="">-- Pilih Kelas --</option>
                  {classes.map((c) => (
                    <option key={c.id} value={c.id}>
                      Kelas {c.class_name} ({c.total_students || 0} siswa)
                    </option>
                  ))}
                </select>
              </div>
              ...
              {(() => {
                const targetClass = classes.find((c) => c.id === bulkDeleteClassId);
                const targetClassName = targetClass ? targetClass.class_name : '';
                const isMatch =
                  targetClassName &&
                  bulkDeleteConfirmationText.trim().toLowerCase() === targetClassName.trim().toLowerCase();
                const canSubmit =
                  bulkDeleteConfirmedCheckbox &&
                  isMatch &&
                  bulkDeleteStudentCount > 0 &&
                  !isBulkDeleting;
                  ...
                  <>
                    <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 text-slate-800 space-y-2">
                      <div className="flex items-center gap-2 text-amber-900 font-bold text-xs">
                        ...
                        <span>Peringatan Keamanan Tindakan Permanen</span>
                      </div>
                      <p className="text-xs text-amber-900/90 leading-relaxed">
                        Apakah Anda yakin ingin menghapus{' '}
                        <strong className="font-black text-rose-700">SEMUA</strong> siswa dari{' '}
                        <strong className="font-black text-slate-900">
                          Kelas {targetClassName || '...'}
                        </strong>
                        ? Tindakan ini tidak dapat dibatalkan.
                      </p>
                      <div className="p-2.5 rounded-xl bg-white/80 border border-amber-200 text-xs font-bold flex items-center justify-between">
                        <span className="text-slate-600">Jumlah siswa yang akan dihapus:</span>
                        <span className="text-sm font-black text-rose-700">
                          {bulkDeleteStudentCount} Siswa
                        </span>
                      </div>
                      {bulkDeleteStudentCount === 0 && bulkDeleteClassId && (
                        <p className="text-[11px] font-semibold text-slate-500 italic">
                          ℹ️ Kelas ini saat ini tidak memiliki data siswa aktif.
                        </p>
                      )}
                    </div>

                    {/* Step 1: Checkbox */}
                    <label className="flex items-start gap-2.5 cursor-pointer select-none p-3 rounded-xl border border-slate-200 hover:bg-slate-50 transition">
                      <input
                        type="checkbox"
                        checked={bulkDeleteConfirmedCheckbox}
                        onChange={(e) => setBulkDeleteConfirmedCheckbox(e.target.checked)}
                        disabled={isBulkDeleting || bulkDeleteStudentCount === 0}
                        className="w-4 h-4 text-rose-600 rounded mt-0.5 border-slate-300 focus:ring-rose-500"
                      />
                      <span className="text-xs font-bold text-slate-700">
                        Saya memahami bahwa seluruh data siswa di kelas ini akan dihapus.
                      </span>
                    </label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 mb-1">
                        Ketik <span className="font-mono font-black text-rose-700 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200">"{targetClassName || 'nama kelas'}"</span> untuk melanjutkan:
                      </label>
                      <input
                        type="text"
                        value={bulkDeleteConfirmationText}
                        onChange={(e) => setBulkDeleteConfirmationText(e.target.value)}
                        disabled={isBulkDeleting || !bulkDeleteConfirmedCheckbox}
                        placeholder={targetClassName ? `Ketik persis: ${targetClassName}` : 'Pilih kelas terlebih dahulu'}
                        className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-black text-slate-900 placeholder:font-normal focus:outline-none focus:ring-2 focus:ring-rose-500 disabled:bg-slate-100"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100">
                      ...
                      <button
                        ...
                        disabled={!canSubmit}
                        className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white text-xs font-black shadow-md shadow-rose-600/20 transition flex items-center gap-1.5"
                      >
                        {isBulkDeleting ? (
                          <>
                            <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                            <span>Menghapus {bulkDeleteStudentCount} Siswa...</span>
                          </>
                        ) : (
                          <>
                            <Trash2 className="w-4 h-4" />
